...
system(paste0('mkdir -p ',folder,' ',folder,sub_f))
...
facs=read_csv('~/Desktop/Projects/Rho/FACS/AphRO.csv',col_names = F)
...
results=read_tsv('~/Desktop/Projects/Rho/Results_periodic_simulationXY_1kb.tsv')
...
to_norm = read_tsv('~/Desktop/Projects/Rho/All_normalised_Tracks_50adj_normalised_with_sim_periodic_1kbXY_ATRiadd.tsv') %>% group_by(Condition, phase, rep, Normalize_factor) %>% summarise(th = min(th))
